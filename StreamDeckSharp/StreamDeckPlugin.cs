...
namespace StreamDeckSharp
{
    public class StreamDeckPlugin<T>
    {
        ...
        public async Task Run(StreamDeckConfig cfg)
        {
            Console.WriteLine(">>> StreamDeck Plugin Starting");
            ...
            if((_args != null) && _args.Any())
            {
                ...
                relDeploy = _args.Contains("-releaseBundle");

                Console.WriteLine($">>> Config Check? {checkConfig}");
                Console.WriteLine($">>> Deploy Dev? {devDeploy}");
                Console.WriteLine($">>> Package Release? {relDeploy}");
            }
            ...
            if(checkConfig)
            {
                ...
                var appConfigOk = await CheckAppSettingsJson(cfg);
                if(appConfigOk)
                    Console.WriteLine(">>> AppSettings.json Ok");
                else 
                    Console.WriteLine(">>> AppSettings.json drifted");

                var manifestOk = await CheckManifestJson(cfg);
                if(manifestOk)
                    Console.WriteLine(">>> Manifest.json Ok");
                else
                    Console.WriteLine(">>> Manifest.json drifted");

                var actionOk = await CheckActionCs(cfg);
                if(actionOk)
                    Console.WriteLine(">>> GeneratedAction.cs Ok");
                else
                    Console.WriteLine(">>> GeneratedAction.cs drifted");

                var actionStubOk = await CheckActionStubCs(cfg);
                if(actionStubOk)
                    Console.WriteLine(">>> ActionStub.cs Ok");
                else
                    Console.WriteLine(">>> ActionStub.cs drifted");

                var serviceStubOk = await CheckServiceStubCs(cfg);
                if(serviceStubOk)
                    Console.WriteLine(">>> ServiceStub.cs Ok");
                else
                    Console.WriteLine(">>> ServiceStub.cs drifted");

                var modelOk = await CheckModelCs(cfg);
                if(modelOk)
                    Console.WriteLine(">>> GeneratedModel.cs Ok");
                else
                    Console.WriteLine(">>> GeneratedModel.cs drifted");

                var actionStatesOk = await CheckActionStatesCs(cfg);
                if(actionStatesOk)
                    Console.WriteLine(">>> ActionStates.cs Ok");
                else
                    Console.WriteLine(">>> ActionStates.cs drifted");

				var propertyInspectorHtmlOk = await CheckPropertyInspectorHtml(cfg);
				if(propertyInspectorHtmlOk)
					Console.WriteLine(">>> PropertyInspector.html Ok");
				else
					Console.WriteLine(">>> PropertyInspector.html drifted");

				var propertyInspectorCssOk = await CheckPropertyInspectorCss(cfg);
				if(propertyInspectorCssOk)
					Console.WriteLine(">>> PropertyInspector.css Ok");
				else
					Console.WriteLine(">>> PropertyInspector.css drifted");

				var propertyInspectorJsOk = await CheckPropertyInspectorJs(cfg);
				if(propertyInspectorJsOk)
					Console.WriteLine(">>> PropertyInspector.js Ok");
				else
					Console.WriteLine(">>> PropertyInspector.js drifted");

				var checkConfigOk = appConfigOk && manifestOk && actionOk && actionStubOk && serviceStubOk && modelOk
                    && actionStatesOk && propertyInspectorHtmlOk && propertyInspectorCssOk && propertyInspectorJsOk;

                if(!checkConfigOk)
                {
                    Console.WriteLine(">>> StreamDeck configuration has changed");
                    Console.WriteLine(">>> Please rebuild project");
                    Environment.Exit(-1);
                }
            }

            if(devDeploy || relDeploy)
            {
                Console.WriteLine(">>> Deploying to Dev");
                var devDeployOk = await DoDevelopmentDeployment(cfg);

                if(!devDeployOk)
                {
                    Console.WriteLine(">>> Development deployment has failed");
                    Environment.Exit(-1);
                }
            }
            ...
            if(relDeploy)
            {
                Console.WriteLine(">>> Packaging Release");
                var relDeployOk = await DoReleaseDeployment(cfg);

                if(!relDeployOk)
                {
                    Console.WriteLine(">>> Release deployment has failed");
                    Environment.Exit(-1);
                }
            }

            if(checkConfig || devDeploy || relDeploy)
                Environment.Exit(0);

            using var config = ConfigurationBuilder.BuildDefaultConfiguration(_args);
            await ConnectionManager
                .Initialize(_args, config.LoggerFactory)
                .RegisterAllActions(typeof(T).Assembly)
                .StartAsync();
        }

        private static async Task<bool> CheckJsonFile(StreamDeckConfig cfg, string filename, dynamic jsonObj)
        {
            ...
            if(File.Exists(filename))
            {
                ...
                var existingObj = JsonSerializer.Deserialize<dynamic>(content);
                var existingJson = JsonSerializer.Serialize(existingObj);

                createFile = existingJson.Trim() != json.Trim();
            }

            if(createFile)
            {
				WriteContentFile(cfg, filename, json);
                return false;
            }
		...
		private static void WriteContentFile(StreamDeckConfig cfg, string filename, string content)
		{
			if(File.Exists(filename))
			...
			var pluginProjectFile = $"{name}.csproj";
			Console.WriteLine($"    >>> Project CsProj: {pluginProjectFile}");

			var projectContent = File.ReadAllText(pluginProjectFile);
			var projectXML = XDocument.Parse(projectContent);
			var projectNode = projectXML.Element("Project");
			if(projectNode == null)
				throw new InvalidOperationException($"    >>> Project node not found in {pluginProjectFile}");
			var itemGroupNodes = projectNode.Elements("ItemGroup");
			if(itemGroupNodes == null)
				throw new InvalidOperationException($"    >>> ItemGroup nodes not found in {pluginProjectFile}");

			foreach(var itemGroup in itemGroupNodes)
			{
			...
			var copyToOutputDirectoryElement = new XElement("CopyToOutputDirectory");
			copyToOutputDirectoryElement.SetValue("PreserveNewest");
			var contentElement = new XElement("Content");
			contentElement.SetAttributeValue("Include", filename);
			contentElement.Add(copyToOutputDirectoryElement);
			var itemGroupElement = new XElement("ItemGroup");
			itemGroupElement.Add(contentElement);
			projectNode.Add(itemGroupElement);
			File.WriteAllText(pluginProjectFile, projectXML.ToString());
		}

		private static async Task<bool> CheckAppSettingsJson(StreamDeckConfig cfg)
        {
            return await CheckJsonFile(cfg, "appsettings.json", new
            {
                Serilog = new
                {
        ...
        private static async Task<bool> CheckManifestJson(StreamDeckConfig cfg)
        {
            ...
            return await CheckJsonFile(cfg, "manifest.json", new
            {
                Actions = new[] {
                    new {
                        Icon = cfg.ActionIcon,
                        Name = cfg.ActionName,
                        States = cfg.StateIcons.Select(_ => new {
                            Image = _.Value,
                            TitleAlignment = "middle",
                            FontSize = "16"
                        }).ToArray(),
                        SupportedInMultiActions = false,
                        Tooltip = cfg.ActionDescription,
                        UUID = cfg.ActionId
                    }
                },
                ...
                Description = cfg.PluginDescription,
                Name = cfg.PluginName,
                Icon = cfg.PluginIcon,
                URL = cfg.PluginUrl,
        ...
        private static async Task<bool> CheckCsFile(string folder, string file, string csCode)
        {
            ...
            {
                var content = File.ReadAllText(filename);
                createFile = content.Trim() != csCode.Trim();
            }
        ...
        private static async Task<bool> CheckActionCs(StreamDeckConfig cfg)
        {
            ...
            return await CheckCsFile("GeneratedFiles", $"{name}Action.cs", $@"using StreamDeckLib;
using StreamDeckPluginBase;

namespace {name}
{{
    [ActionUuid(Uuid = ""{cfg.ActionId}"")]
    public partial class {name}Action : StreamDeckAction<{name}Model>
    {{
        public {name}Action()
            : base({cfg.UpdateFrequencySeconds})
        {{
            Initialize().Wait();
        }}
    }}
...
namespace {name}
{{
    public class {name}Model : StreamDeckModel
    {{
        {properties}
    }}
}}");
        ...
        private static async Task<bool> CheckActionStatesCs(StreamDeckConfig cfg)
        {
            ...
            int idx = 0;
            foreach(var kvp in cfg.StateIcons)
                values.Add($"{kvp.Key}={idx++}");

            return await CheckCsFile("GeneratedFiles", $"ActionStates.cs", $@"namespace {name}
{{
    ...
    {{
        ...
        private static async Task<bool> CheckActionStubCs(StreamDeckConfig cfg)
        {
            var name = GetProjectName(cfg);

            var initialActionFilename = $"{name}Action.cs";
            if(!File.Exists(initialActionFilename))
            {
                var initialAction = $@"using StreamDeckLib.Messages;
using StreamDeckPluginBase;
using System;
    ...
    {{
        ...
        {{
            ...
            await Task.CompletedTask;
                ...
                File.WriteAllText(initialActionFilename, initialAction);
        ...
        private static async Task<bool> CheckServiceStubCs(StreamDeckConfig cfg)
        {
            var name = GetProjectName(cfg);

            var initialServiceFilename = $"{name}Service.cs";
            if(!File.Exists(initialServiceFilename))
            {
                var initialService = $@"using Microsoft.Extensions.Logging;
...
{{
    ...
    {{
        ...
        public void Initialize(ILogger logger, {name}Model settingsModel)
        {{
			if(_initialized)
				return;
            ...
            _initialized = true;
                ...
                File.WriteAllText(initialServiceFilename, initialService);
		...
		private static async Task<bool> CheckPropertyInspectorHtml(StreamDeckConfig cfg)
		{
			...
			var propertyInspectorFolder = CheckPropertyInspectorFolder();

			var isOkay = true;
			var htmlFilename = "PropertyInspector.html";
			var propertyList = cfg.Configurations.Select(_ => $@"<div class=""sdpi-item"" id=""{_.Value}"" type=""field"" >
...
<head>
    <meta charset=""utf-8"" />
    <title>{cfg.ActionName} Property Inspector</title>
...
<body>
    ...
    <script src=""PropertyInspector.js"" ></script>
			...
			var createHtml = true;
			var htmlFullFilename = Path.Combine(propertyInspectorFolder, htmlFilename);
			if(File.Exists(htmlFullFilename))
			{
				var existingHtml = File.ReadAllText(htmlFullFilename);
				createHtml = existingHtml.Trim() != html.Trim();
			}

			if(createHtml)
			{
				WriteContentFile(cfg, htmlFullFilename, html);
				isOkay = false;
			}

			return await Task.FromResult(isOkay);
		}
		...
		private static async Task<bool> CheckPropertyInspectorCss(StreamDeckConfig cfg)
		{
			...
			var isOkay = true;
			var cssFilename = "PropertyInspector.css";
			var css = $@"html {{
	--sdpi-bgcolor: #2D2D2D;
...
.noInnerMargins * {{
	...
	padding: 0;
			...
			var createCss = true;
			var cssFullFilename = Path.Combine(propertyInspectorFolder, cssFilename);
			if(File.Exists(cssFullFilename))
			{
				var existingCss = File.ReadAllText(cssFullFilename);
				createCss = existingCss.Trim() != css.Trim();
			}

			if(createCss)
			{
				WriteContentFile(cfg, cssFullFilename, css);
				isOkay = false;
			}
		...
		private static string CheckPropertyInspectorFolder()
		{
			var propertyInspectorFolder = "PropertyInspector";
			var folderInfo = new DirectoryInfo(propertyInspectorFolder);
			if(!folderInfo.Exists)
				folderInfo.Create();

			return propertyInspectorFolder;
		}
...
const setSettings = (value, param) => {{
    if (websocket) {{
        ...
        websocket.send(JSON.stringify(json));
			...
			var createJs = true;
			var jsFullFilename = Path.Combine(propertyInspectorFolder, jsFilename);
			if(File.Exists(jsFullFilename))
			{
				var existingJs = File.ReadAllText(jsFullFilename);
				createJs = existingJs.Trim() != js.Trim();
			}

			if(createJs)
			{
				WriteContentFile(cfg, jsFullFilename, js);
				isOkay = false;
			}
		...
		private static async Task<bool> DoDevelopmentDeployment(StreamDeckConfig cfg)
        {
            Console.WriteLine(">>> Development deployment starting");
            ...
            if(projectNode == null)
                throw new InvalidOperationException($"    >>> Project node not found in {pluginProjectFile}");
            var propertyGroupNode = projectNode.Element("PropertyGroup");
            if(propertyGroupNode == null)
                throw new InvalidOperationException($"    >>> Project node not found in {pluginProjectFile}");
            var targetFrameworkNode = propertyGroupNode.Element("TargetFramework");
            if(targetFrameworkNode == null)
                throw new InvalidOperationException($"    >>> Project node not found in {pluginProjectFile}");
            var targetFrameworkName = targetFrameworkNode.Value;
                
            Console.WriteLine($"    >>> Target Framework: {targetFrameworkName}");

            var streamDeckExePath = $"{Environment.ExpandEnvironmentVariables("%ProgramFiles%")}\\Elgato\\StreamDeck\\StreamDeck.exe";
            Console.WriteLine($"    >>> StreamDeck Exe: {streamDeckExePath}");

            var binPath = $"bin\\Debug\\{targetFrameworkName}";
            Console.WriteLine($"    >>> Bin Path: {binPath}");

            var pluginId = cfg.ActionId;
            Console.WriteLine($"    >>> Plugin Id: {pluginId}");

            var pluginsPath = $"{Environment.ExpandEnvironmentVariables("%AppData%")}\\Elgato\\StreamDeck\\Plugins";
            Console.WriteLine($"    >>> Plugins Path: {pluginsPath}");

            var destPath = $"{pluginsPath}\\{pluginId}.sdPlugin";
            Console.WriteLine($"    >>> Dest Path: {destPath}");

            var pluginName = cfg.ActionId;
            Console.WriteLine($"    >>> Plugin Name: {pluginName}");
            Console.WriteLine($"    >>> Killing processes");
            Console.WriteLine($"    >>> Killing StreamDeck");
            foreach(var process in Process.GetProcessesByName("StreamDeck"))
                process.Kill();
            Console.WriteLine($"    >>> StreamDeck killed");
            Console.WriteLine($"    >>> Killing Plugin");
            foreach(var process in Process.GetProcessesByName(pluginName))
                process.Kill();
            Console.WriteLine($"    >>> Plugin killed");
            Console.WriteLine($"    >>> Sleeping");
            Thread.Sleep(5000);
            Console.WriteLine($"    >>> Deleting Dest Path");
            var dir = new DirectoryInfo(destPath);
            if(dir.Exists) {
                dir.Delete(true);
            }
            Console.WriteLine($"    >>> Dest Path Deleted");
            Console.WriteLine($"    >>> Creating Dest Path");
            dir.Create();

            var sourcePath = new DirectoryInfo(binPath);
            Console.WriteLine($"    >>> Copying from {binPath}");
            var files = sourcePath.GetFiles("*", SearchOption.AllDirectories);
            foreach(var file in files)
            {
                ...
                if(targetInfo.Directory == null)
                    throw new InvalidOperationException();
                if(!targetInfo.Directory.Exists)
                    targetInfo.Directory.Create();
                file.CopyTo(target, true);
            }
            Console.WriteLine($"    >>> Sources Copied");

            Console.WriteLine($"    >>> Starting StreamDeck");
            Process.Start(new ProcessStartInfo { 
                FileName = streamDeckExePath,
                UseShellExecute = true
            });
            Console.WriteLine($"    >>> StreamDeck Started");

            Console.WriteLine(">>> Development deployment complete");

            return await Task.FromResult(true);
        }

        private static async Task<bool> DoReleaseDeployment(StreamDeckConfig cfg)
        {
            ...
            var releasePath = Path.Combine(dir, "..\\Dist");
            var releasePackage = $"{releasePath}\\{cfg.ActionId}.streamDeckPlugin";
            if (File.Exists(releasePackage) ){
                Console.WriteLine($"    >>> Deleting existing release package");
                File.Delete(releasePackage);
            }
            Console.WriteLine($"    >>> Deleting Dest Path");
            ...
            Console.WriteLine($"    >>> Running distribution tool: {distTool}");
            var args = $@"-b -i ""{cfg.ActionId.ToLower()}.sdPlugin"" -o ""{releasePath}""";
            Console.WriteLine($"    >>> Distribution tool args: {args}");
            var p = Process.Start(distTool, args);
            Console.WriteLine($"    >>> Distribution tool running");
            p.WaitForExit();
            Console.WriteLine($"    >>> Distribution tool complete");

            Directory.SetCurrentDirectory(dir);

            Console.WriteLine(">>> Release deployment complete");
